...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    ...
    if (!ENV.oAuthServerUrl) {
      console.error(
        "[OAuth] ERRO: OAUTH_SERVER_URL não está configurado! Defina a variável de ambiente OAUTH_SERVER_URL.",
      )
    }
  }
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    if (!user) {
      try {
        ...
        console.error("[Auth] Falha ao sincronizar usuário do OAuth:", error)
